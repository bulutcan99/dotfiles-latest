        ...
        callback = function(args)
            ...
            vim.keymap.set("n", keymaps.open_tmux_pane, function()
                ...
                if curr_entry and curr_entry.fs_type == "directory" then
                    ...
                    require("config.keymaps").tmux_pane_function(
                        curr_entry.path
                    )
                else
                    -- Notify if not a directory or no entry is selected
                    vim.notify(
                        "Not a directory or no entry selected",
                        vim.log.levels.WARN
                    )
                end
            ...
            vim.keymap.set(
                ...
                end,
                {
                    ...
                    noremap = true,
                    silent = true,
                    desc = "[P]Copy file/directory to clipboard",
                }
            )
            ...
            vim.keymap.set(
                ...
                function()
                    ...
                    if curr_entry then
                        ...
                        local zip_path =
                            string.format("/tmp/%s_%s.zip", name, timestamp) -- Path in macOS's tmp directory
                        -- Create the zip file
                        ...
                        if vim.v.shell_error ~= 0 then
                            vim.notify(
                                "Failed to create zip file: " .. result,
                                vim.log.levels.ERROR
                            )
                            return
                        end
                        ...
                        if vim.v.shell_error ~= 0 then
                            vim.notify(
                                "Failed to copy zip file to clipboard: "
                                    .. copy_result,
                                vim.log.levels.ERROR
                            )
                            return
                        ...
                        vim.notify(
                            "Zipped and copied to clipboard: ",
                            vim.log.levels.INFO
                        )
                    else
                        vim.notify(
                ...
                end,
                {
                    ...
                    noremap = true,
                    silent = true,
                    desc = "[P]Zip and copy to clipboard",
                }
            )
            ...
            vim.keymap.set(
                ...
                function()
                    ...
                    if not curr_entry then
                        vim.notify(
                            "Failed to retrieve current entry in mini.files.",
                            vim.log.levels.ERROR
                        )
                        return
                    end
                    local curr_dir = curr_entry.fs_type == "directory"
                            and curr_entry.path
              ...
              on error
                return "error"
                    ...
                    local output = vim.fn.system(
                        "osascript -e " .. vim.fn.shellescape(script)
                    ) -- Execute AppleScript command
                    if vim.v.shell_error ~= 0 or output:find("error") then
                        vim.notify(
                            "Clipboard does not contain a valid file or directory.",
                            vim.log.levels.WARN
                        )
                        return
                    end
            ...
            vim.keymap.set(
                ...
                end,
                {
                    ...
                    noremap = true,
                    silent = true,
                    desc = "[P]Copy relative path to clipboard",
                }
            )
                    ...
                    if curr_entry then
                        ...
                        vim.defer_fn(function()
                            vim.system({
                                "osascript",
                                "-e",
                                'tell application "qlmanage" to activate',
                            })
                        end, 200)
                    else
